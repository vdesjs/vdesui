<template>
  <div class="index">
    ...
    <div class="index-components">
      <ol v-for="api in data.apis" :key="api.name">
        <template v-if="api.demoShow != false">
          <li>{{ api.name }}</li>
          <ul>
            <template v-for="_package in api.packages" :key="_package">
              <li>
                <router-link :to="replaceRoutePath(_package.path)">
                  {{ _package.name }}&nbsp;&nbsp;
                </router-link>
                <img
                  class="right-arrow"
